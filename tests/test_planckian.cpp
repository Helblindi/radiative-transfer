...
//----------------------------------
  ...
  Eigen::VectorXd x_edge(num_cells+1), x_ave(num_cells); //cell edge and average coordinates
...
//------------------------------------------------
  Eigen::VectorXd e_edge(num_groups+1), e_ave(num_groups), de_ave(num_groups);  //Group edge and average energies and average group widths in kev
...
//---------------------------------------------------------
  ...
  Eigen::MatrixXd energy_discretization(num_groups, 2);  // Creates a vector of pairs of group energies edges for the Planck routines
...
//----------------------------------------------------
  ...
  Planck planck;
  Eigen::VectorXd B(num_groups), dBdT(num_groups);
...
//---------------------------------------------
  ...
  {
   cout << left << setw(7)  << g 
	      << left << setw(14) << energy_discretization(g,0)
	      << left << setw(14) << energy_discretization(g,1)
	      << left << setw(14) << B(g)
...
//------------------------------
  ...
  Eigen::VectorXd ukappa(num_groups), kappa(num_groups);  // Unnormalized and final opacities respectively
  Eigen::VectorXd ckappa(num_groups);                     // Opacities evaluated at group center energies
...
//--------------------------------------------------------
  ...
  Eigen::VectorXd emis_spec(num_groups);
...
//----------------------------------------
  ...
  Eigen::VectorXd dEB(num_groups), dkapEdE(num_groups), dkapEB(num_groups); 
...
//-----------------------------
  Eigen::VectorXd kappa_edge(num_groups+1);  // Edge opacities
...
//--------------------------------------------
  Eigen::VectorXd cor1(num_groups), cor2(num_groups), cor3(num_groups); // correction terms 1, 2, and 3
