...
template<int num_groups>
Solver<num_groups>::Solver(Eigen::Ref<Eigen::MatrixXd> psi_mat,
   ...
   F_ref(F)
{
   ...
   ends.resize(ctv::M,ctv::N,2);      
   prev_ends.resize(ctv::M,ctv::N,2);
   half_ends.resize(ctv::M,ctv::N,2); 
...
{
   int _M = psi_mat_ref.rows(), _N = psi_mat_ref.cols();
   ...
   for (int i = 0; i < _N; i++)
   {
      ...
      for (int j = 0; j < _M; j++)
      {
         phi_ref[i] += ctv::G_w[j] * psi_mat_ref(j,i);
      }
   }
}
...
void Solver<num_groups>::compute_radiative_flux()
{
   ...
   for (int i = 0; i < _N; i++)
   {
      ...
      for (int j = 0.; j < _M; j++)
      {
         F_ref[i] += ctv::G_x[j] * ctv::G_w[j] * psi_mat_ref(j,i);
      }
   }
}
...
template<int num_groups>
double Solver<num_groups>::compute_balance()
{
   double mu = 0.,
          j_half_minus = 0., 
          j_half_plus = 0.,
   ...
   for (int i = 0; i < ctv::M; i++)
   {
      ...
      if (mu < 0.)
      {
         j_half_minus -= ends(i,0,0) * mu * ctv::G_w[i]; // psi_1/2
         jN_half_minus -= ends(i,ctv::N-1,1) * mu * ctv::G_w[i]; // psi_N+1/2
      }
      else 
      {
         j_half_plus += ends(i,0,0) * mu * ctv::G_w[i]; // psi_1/2
         jN_half_plus += ends(i,ctv::N-1,1) * mu * ctv::G_w[i]; // psi_N+1/2
      }
   }

   for (int i = 0; i < ctv::N; i++)
   {
      _abs += rho_vec[i] * kappa_vec[i] * phi_ref[i] * ctv::dx;
      _src += rho_vec[i] * kappa_vec[i] * ctv::a * ctv::c * pow(temperature[i],4) * ctv::dx;
   }
...
void Solver<num_groups>::solve()
{
   for (int _it = 0; _it < ctv::_max_timesteps; _it++)
   {
      ...
      for (int i = 0; i < ctv::M; i++)
      {
         ...
         for (int j = 0; j < ctv::N; j++)
         {
            if (mu < 0)
            {
               ...
               switch(ctv::ts_method) {
                  case 1: // BE
                  {
                     double const_A = 1. + ctv::c*ctv::dt*rho_vec[cell_j] * kappa_vec[cell_j];
                     double const_B = ctv::c*ctv::dt*mu;
                     double const_C = ctv::c*ctv::dt*rho_vec[cell_j] * kappa_vec[cell_j] * ctv::a * ctv::c / (4 * M_PI);
                     ...
                     _temp_val = const_C * ctv::dx * pow(temperature[cell_j], 4) / 2.;
                     _rhs(0) = _temp_val + ctv::dx * ends(i,cell_j,0) / 2.;
                     _rhs(1) = _temp_val - (const_B * local_bdry) + ctv::dx * ends(i,cell_j,1) / 2.;
                     ...
                     psi_mat_ref(i,cell_j) = 0.5*(_res[0] + _res[1]);
                  ...
                  case 2: // BDF2
                  {
                     ...
                     _rhs[0] = _temp_val + ((const_C * ctv::dx + const_A) / 2.) * ends(i,cell_j,0) - (const_A / 2.) * ends(i,cell_j,1);
                     _rhs[1] = _temp_val + ((const_C * ctv::dx + const_A) / 2.) * ends(i,cell_j,1) + (const_A / 2.) * ends(i,cell_j,0) - const_A * (half_local_bdry + local_bdry_prev_it);
                     ...
                     _rhs[0] = _temp_val + ((const_C * ctv::dx + 4. * const_B) / 2.) * half_ends(i,cell_j,0) - 2. * const_B * half_ends(i,cell_j,1);
                     _rhs[0] += ((const_B - const_D * ctv::dx) / 2.) * ends(i,cell_j,0) - (const_B / 2.) * ends(i,cell_j,1);
                     _rhs[1] = _temp_val + ((const_C * ctv::dx + 4. * const_B) / 2.) * half_ends(i,cell_j,1) + 2. * const_B * half_ends(i,cell_j,0);
                     _rhs[1] += ((const_B - const_D * ctv::dx) / 2.) * ends(i,cell_j,1) + (const_B / 2.) * ends(i,cell_j,0);
                  ...
                  case 4: // BDF2 Morel-Lou
                  {
                     ...
                     else
                     {
                        ...
                        _temp_val = (const_E * ctv::dx * rho_vec[cell_j] * kappa_vec[cell_j] / 2.) * pow(temperature[cell_j], 4);
                        _rhs[0] = _temp_val + ((const_C * ctv::dx + (const_B / 4.)) / 2.) * half_ends(i,cell_j,0) - (const_B / 8.) * half_ends(i,cell_j,1);
                        _rhs[0] += (((const_B / 4.) - const_D * ctv::dx) / 2.) * ends(i,cell_j,0) - (const_B / 8.) * ends(i,cell_j,1);
                        _rhs[1] = _temp_val + ((const_C * ctv::dx + (const_B / 4.)) / 2.) * half_ends(i,cell_j,1) + (const_B / 8.) * half_ends(i,cell_j,0);
                        _rhs[1] += (((const_B / 4.) - const_D * ctv::dx) / 2.) * ends(i,cell_j,1) + (const_B / 8.) * ends(i,cell_j,0);
            ...
            { // mu > 0
               ...
               switch(ctv::ts_method) {
                  case 1: // BE
                  {
                     double const_A = 1. + ctv::c*ctv::dt*rho_vec[j] * kappa_vec[j];
                     double const_B = ctv::c*ctv::dt*mu;
                     double const_C = ctv::c*ctv::dt*rho_vec[j] * kappa_vec[j] * ctv::a * ctv::c / (4 * M_PI);
                     ...
                     _temp_val = const_C * ctv::dx * pow(temperature[j], 4) / 2.;
                     _rhs[0] = _temp_val + (const_B * local_bdry) + ctv::dx * ends(i,j,0) / 2.;
                     _rhs[1] = _temp_val + ctv::dx * ends(i,j,1) / 2.;
                     ...
                     // put the average of val and local boundary here
                     psi_mat_ref(i,j) = 0.5*(_res[0] + _res[1]);
                  ...
                  case 2: // BDF2
                  {
                     ...
                     _rhs[0] = _temp_val + ((const_C * ctv::dx - const_A) / 2.) * ends(i,j,0) - (const_A / 2.) * ends(i,j,1) + const_A * (half_local_bdry + local_bdry_prev_it);
                     _rhs[1] = _temp_val + ((const_C * ctv::dx - const_A) / 2.) * ends(i,j,1) + (const_A / 2.) * ends(i,j,0);
                     ...
                     _rhs[0] = _temp_val + ((const_C * ctv::dx - 4. * const_B) / 2.) * half_ends(i,j,0) - 2. * const_B * half_ends(i,j,1);
                     _rhs[0] += -1. * ((const_B + const_D * ctv::dx) / 2.) * ends(i,j,0) - (const_B / 2.) * ends(i,j,1);
                     _rhs[0] += const_B * (local_bdry + local_bdry_prev_it + 4. * half_local_bdry);
                     _rhs[1] = _temp_val + ((const_C * ctv::dx - 4. * const_B) / 2.) * half_ends(i,j,1) + 2. * const_B * half_ends(i,j,0);
                     _rhs[1] += -1. * ((const_B + const_D * ctv::dx) / 2.) * ends(i,j,1) + (const_B / 2.) * ends(i,j,0);
                  ...
                  case 4: // BDF2 Morel-Lou
                  {
                     ...
                     else
                     {
                        ...
                        _temp_val = (const_E * ctv::dx * rho_vec[j] * kappa_vec[j] / 2.) * pow(temperature[j], 4);
                        _rhs[0] = _temp_val + ((const_C * ctv::dx - const_B/4.) / 2.) * half_ends(i,j,0) - (const_B / 8.) * half_ends(i,j,1);
                        _rhs[0] += -1. * (((const_B / 4.) + const_D * ctv::dx) / 2.) * ends(i,j,0) - (const_B / 8.) * ends(i,j,1);
                        _rhs[0] += const_B * (local_bdry + (local_bdry_prev_it / 4.) + (half_local_bdry / 4.));
                        _rhs[1] = _temp_val + ((const_C * ctv::dx - (const_B / 4.)) / 2.) * half_ends(i,j,1) + (const_B / 8.) * half_ends(i,j,0);
                        _rhs[1] += -1. * (((const_B / 4.) + const_D * ctv::dx) / 2.) * ends(i,j,1) + (const_B / 8.) * ends(i,j,0);
         ...
         {
            cout << "Final phi for mu " << mu << " is: " << ends(i,ctv::N-1,1) << endl;
         }
